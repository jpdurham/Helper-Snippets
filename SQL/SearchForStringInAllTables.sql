/* 
 * DESCRIPTION: Adds a sproc to your database that allows you to search for a string in
 ...
 * SIDE-EFFECT: This will delete any sproc name [dbo].[SearchForStringInAllTables]
...
IF EXISTS (
   SELECT *
   FROM sys.procedures
   WHERE [object_id] = OBJECT_ID(N'[dbo].[SearchForStringInAllTables]'))
BEGIN
   DROP PROCEDURE [dbo].[SearchForStringInAllTables];
END;
GO

CREATE PROCEDURE [dbo].[SearchForStringInAllTables]
...
BEGIN
   ...
   INSERT INTO #tables SELECT * FROM (
    ...
    BEGIN
        SET @sql = 'INSERT INTO #results SELECT ''SELECT [' + @column_name + '], *  FROM [' + @schema_name 
                 + '].[' +  @table_name + '] WHERE [' + @column_name + ']  LIKE ''''%' + @query_text 
                 + '%'''' '' query, ''' + @schema_name + ''' [schema_name], ''' + @table_name 
                 + ''' [table_name], ''' + @column_name + ''' [column_name], COUNT(-1) row_count FROM [' 
                 + @schema_name + '].[' +  @table_name + '] WHERE [' + @column_name + ']  LIKE ''%' 
                 + @query_text + '%'''; 
